...
const char * const BITCOIN_CONF_FILENAME = "quix.conf";
const char * const BITCOIN_PID_FILENAME = "quixd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "quix" is a composite category enabling all QUIX-related debug output
            if(ptrCategory->count(string("quix"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QUIXCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QUIXCore
    // Mac: ~/Library/Application Support/QUIXCore
    // Unix: ~/.quixcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QUIXCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QUIXCore";
#else
    // Unix
    return pathRet / ".quixcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty quix.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override quix.conf
