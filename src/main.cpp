...
#if defined(NDEBUG)
# error "QUIX Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight < 1900){
        ...
        nSubsidy = nPrevHeight * nPrevHeight * 3 / 1000000 * COIN;
    }else{
        nSubsidy = 300 * COIN;
    }
...
CAmount GetMasternodePayment(int nHeight, CAmount blockValue)
{
    ...
    CAmount ret = blockValue*9/10;

    if(nHeight > nMNPIBlock)                  ret += blockValue / 50;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 1)) ret += blockValue / 50;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 2)) ret += blockValue / 50;
    if(nHeight > nMNPIBlock+(nMNPIPeriod* 3)) ret += blockValue / 50;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(QUIX): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(QUIX): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(QUIX): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(QUIX): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        QUIX Related Inventory Messages
